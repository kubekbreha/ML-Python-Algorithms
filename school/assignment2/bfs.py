...
class BFS:
    ...
    def perform(puzzle, row, col):
        ...
        while (1):  # while the puzzle is not solved search for solution
            ...
            for i in range(row):
                for e in range(col):
                    ...
                    if check not in moves:  # if the node has been visited do nothing
            ...
            if (GameController.is_solved(fifo) == 1):
                break

        ret = []
        for i in range(row):
            for e in range(col):
                # if coordinates occur in any muitple of two then it is the same as not
                if coord.count((str(i) + str(e))) % 2 == 1:
